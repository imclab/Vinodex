...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Winery', fields ['url', 'location', 'name', 'address']
        db.create_unique(u'wine_winery', ['url', 'location', 'name', 'address'])

        # Adding unique constraint on 'Cellar', fields ['owner', 'name']
        db.create_unique(u'wine_cellar', ['owner_id', 'name'])

        # Adding unique constraint on 'Wine', fields ['vintage', 'name', 'winery']
        db.create_unique(u'wine_wine', ['vintage', 'name', 'winery_id'])

        # Adding unique constraint on 'Annotation', fields ['value', 'key', 'bottle']
        db.create_unique(u'wine_annotation', ['value', 'key', 'bottle_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Annotation', fields ['value', 'key', 'bottle']
        db.delete_unique(u'wine_annotation', ['value', 'key', 'bottle_id'])

        # Removing unique constraint on 'Wine', fields ['vintage', 'name', 'winery']
        db.delete_unique(u'wine_wine', ['vintage', 'name', 'winery_id'])

        # Removing unique constraint on 'Cellar', fields ['owner', 'name']
        db.delete_unique(u'wine_cellar', ['owner_id', 'name'])

        # Removing unique constraint on 'Winery', fields ['url', 'location', 'name', 'address']
        db.delete_unique(u'wine_winery', ['url', 'location', 'name', 'address'])
    ...
    models = {
        ...
        u'wine.cellar': {
            ...
            'owner': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'cellars'", 'to': u"orm['wine.UserProfile']"}),
            'photo': ('django.db.models.fields.URLField', [], {'max_length': '200', 'null': 'True', 'blank': 'True'})
        },
        u'wine.userprofile': {
        ...
        u'wine.wine': {
            'Meta': {'unique_together': "(['name', 'winery', 'vintage'],)", 'object_name': 'Wine'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            ...
            'winery': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'wines'", 'null': 'True', 'to': u"orm['wine.Winery']"})
        },
        u'wine.winery': {
